window.addEventListener("DOMContentLoaded", function () {
  ...
  function makeSlideHtml() {
    ...
    for (let i = 0; i < originData.length; i++) {
      ...
      for (let j = 0; j < obj.상태.length; j++) {
        tag += `<div class="live_state">
            <span class="live_wait_${obj.상태[j].스타일}">${obj.상태[j].텍스트}</span>`;
      }
      tag += `</div>
      ...
      if (obj.날짜 && obj.시간) {
        tag += `<div class="live_day_date">${obj.날짜}</div>
            <div class="live_day_time">${obj.시간}</div>`;
      }
      tag += `
      </div>`;

      if (obj.상세정보이미지 && obj.상세정보alt && obj.상세정보제목) {
        tag += `<div class="live_detail">
            ...
            <p class="live_detail_title">${obj.상세정보제목}</p>
          </div>`;
      }
